    ...
    >
      ...
      >
        <div
          ...
          @click.stop
        >
          <div class="px-6 py-5 bg-zinc-800/30 border-b border-zinc-700/50 flex items-center justify-between shrink-0">
            ...
            <button @click="emit('close')" class="text-zinc-500 hover:text-white hover:bg-white/10 p-2 rounded-lg transition-colors">
              <X class="w-5 h-5" />
            </button>
          </div>
          ...
          <div class="p-8 space-y-8 overflow-y-auto custom-scrollbar bg-zinc-900/50">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="bg-zinc-950 p-5 rounded-xl border border-zinc-800/60 flex flex-col justify-between h-32">
                <span class="text-xs font-bold text-zinc-500 uppercase tracking-widest">Estimativa</span>
                <div class="flex items-end gap-2">
                  <span class="text-5xl font-mono font-light text-zinc-200 tracking-tighter">{{ estimatedTokens }}</span>
                  <span class="text-sm text-zinc-500 mb-1.5 font-medium">tokens</span>
                </div>
              </div>

              <div class="bg-zinc-950 p-5 rounded-xl border border-zinc-800/60 flex flex-col justify-between h-32 relative overflow-hidden group">
                 <div class="absolute top-0 right-0 w-24 h-24 bg-white/5 rounded-full blur-3xl -translate-y-10 translate-x-10 group-hover:bg-white/10 transition-all"></div>
                 ...
                 <div class="flex justify-between items-start z-10">
                    ...
                    <span 
                      ...
                      :class="score ? 'bg-zinc-900 border-zinc-700 text-zinc-300' : 'bg-zinc-900 border-zinc-800 text-zinc-600'"
                    >
                        {{ scoreLabel }}
                    </span>
                 </div>
                 ...
                 <div class="z-10 w-full">
                    <div class="flex items-end gap-1.5 mb-3">
                        <span class="text-4xl font-bold tracking-tighter" :class="score ? 'text-white' : 'text-zinc-700'">
                            {{ score !== null ? score : '--' }}
                        </span>
            ...
            <div v-if="analysisResult && analysisResult.length > 0" class="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div class="flex items-center gap-2 mb-4">
                  <Sparkles class="w-4 h-4 text-fuchsia-500" /> 
                  <h4 class="text-zinc-100 font-semibold text-sm uppercase tracking-wider">Parecer Técnico</h4>
                </div>
                  ...
                  >
                    ...
                    <div class="text-sm leading-relaxed text-zinc-300">
                      <template v-if="formatItem(item).title">
                         <span class="text-zinc-100 font-bold block mb-1 text-base">{{ formatItem(item).title }}</span>
            ...
            <div v-else-if="!isAnalyzing" class="py-12 text-center opacity-30 border-2 border-dashed border-zinc-800 rounded-xl">
          ...
          <div class="px-6 py-5 bg-zinc-950/80 border-t border-white/5 flex justify-end gap-3 shrink-0 backdrop-blur-md">
            <button
              @click="emit('close')"
